...
def get_movies(filters, page, movies_per_page):
    ...
    movies = cursor.skip(max(0, movies_per_page * page)).limit(movies_per_page)
...
def get_movie(id):
    ...
    try:
        ...
        movie = next(db.movies.aggregate(pipeline))
        return movie
...
def add_user(name, email, hashedpw):
    ...
    try:
        ...
        users_wc = db.users.with_options(write_concern=WriteConcern(w='majority'))
        users_wc.insert_one({
            'name': name,
            'email': email,
            'password': hashedpw,
